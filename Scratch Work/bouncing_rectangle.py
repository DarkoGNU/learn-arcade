...
python -m arcade.example.bouncing_rectangle
"""
...
class Item:
    ...
    def update(self):
        ...
        # Check if we need to bounce off left/right edge
        if self.center_x > SCREEN_WIDTH - RECT_WIDTH / 2 or self.center_x < RECT_WIDTH / 2:
            self.change_x *= -1
        # Check if we need to bounce off top/bottom edge
        if self.center_y > SCREEN_HEIGHT - RECT_HEIGHT / 2 or self.center_y < RECT_HEIGHT / 2:
            self.change_y *= -1
